...
const render = () => {
    const table = document.getElementById('contacts') || document.createElement('table');
    table.id = "contacts";
    table.innerHTML = null;

    contactList.filter(value => {
        return (value.contactName.includes(search) || value.contactPhone.includes(search) || value.contactEmail.includes(search));
        ...
        deleteButton.className = "btn btn-outline-danger pt-0 pb-0 pl-1 pr-1 mb-2";
        ...
        favoriteButton.className = "btn btn-outline-warning pt-0 pb-0 pl-1 pr-1 mb-2";
        favoriteButton.innerHTML = null;
        ...
        editButton.className = "btn btn-outline-secondary pt-0 pb-0 pl-1 pr-1 mb-2";
        ...
        const cP = "📞: " + contact.contactPhone;
        const cE = "✉️: " + contact.contactEmail;
        ...
        editButton.addEventListener('click', event => {
            if (event.target.tagName === "BUTTON") {
                if (editContactName.style.display === "none" && editContactPhone.style.display === "none" && editContactEmail.style.display === "none" && editNow.style.display === "none") {
        ...
        favoriteButton.addEventListener('click', event => {
            if (event.target.tagName === "BUTTON" && contactList[index].favorite === false) {
                contactList[index].favorite = true;
                window.localStorage.setItem("contactList", JSON.stringify(contactList));
                render();
            } else if (event.target.tagName === "BUTTON" && contactList[index].favorite === true) {
        ...
        editNow.addEventListener('click', event => {
            if (event.target.tagName === "BUTTON") {
                const editContactNameInput = document.getElementById('editContactName' + index).value;
                const editContactPhoneInput = document.getElementById('editContactPhone' + index).value;
                const editContactEmailInput = document.getElementById('editContactEmail' + index).value;
    ...
    if (event.target.tagName === "BUTTON") {
        ...
        switch (event.target.name) {
            ...
            case "startSearch":
                ...
                // addEventListener ('input') - live search
                break;
            default:
